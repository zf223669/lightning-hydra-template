...
from src.pymo.data import Joint, MocapData
from src.pymo.preprocessing import *
from src.pymo.writers import *
from sklearn.preprocessing import StandardScaler
import joblib as jl
from transformers import BertTokenizer, BertModel
from src.DataPrepare.parse_json_transcript import encode_json_transcript_with_bert, \
    encode_json_transcript_with_bert_DEPRECATED
...
def slice_data_tri(data, window_size, overlap):
    nframes = data.shape[0]
    overlap_frames = (int)(overlap * window_size)
...
def import_data_tri(file, motion_path, speech_data, transcript_path, mirror=False, start=0, end=None):
    """Loads a file and concatenate all features to one [time, features] matrix.
        NOTE: All sources will be truncated to the shortest length, i.e. we assume they
        are time synchronized and has the same start time."""
    suffix = ""
    ...
    concat_data = align_tri(motion_data, control_data)
...
def import_and_slice_tri(files, motion_path, speech_data, transcript_path, slice_window, slice_overlap, mirror=False,
                         start=0, end=None):
...
if __name__ == "__main__":
    ...
    "MG-R": gesture radius
    "MG-V": hand speech(velocity)
    "MG-H": right-hand hand height
    "MS-S" the degree of gesture symmetry
    '''
    if len(sys.argv) == 1:
        style_path = None
    elif len(sys.argv) == 2:
    ...
    print(f"files[] = {files}".format(files))

    motion_feature = 'joint_rot'
    speech_feature = 'melspec'
    transcripts_feature = 'transcripts'
    ...
    transcripts_path = os.path.join(path, f'{transcripts_feature}')
    ...
    test_split = 1
    #
    train_motion, train_ctrl = import_and_slice_tri(train_files, motion_path, speech_path, transcripts_path,
                                                    slice_win_train,
                                                    window_overlap, mirror=True)
    val_motion, val_ctrl = import_and_slice_tri(held_out, motion_path, speech_path, transcripts_path, slice_win_train,
                                                window_overlap, mirror=True, start=0, end=val_test_split)

    # train_motion, train_ctrl = import_and_slice(train_files, motion_path, speech_path, style_path, slice_win_train,
    #                                             window_overlap, mirror=True)
    # val_motion, val_ctrl = import_and_slice(held_out, motion_path, speech_path, style_path, slice_win_train,
    #                                         window_overlap, mirror=True, start=0, end=val_test_split)
    #
    ...
    #                                         mirror=False, start=0, end=val_test_split)
    # test_motion, test_ctrl = import_and_slice(held_out, motion_path, speech_path, style_path, slice_win_test, 0,
    #                                           mirror=False, start=val_test_split)
    #
